  ...
  case bbM of
    Just (extractPrim ->  Just (BlackBox pNm _ _ _ _ _ inc templ)) -> do
      let usedArgs | isFromInt pNm
                   = [0,1,2]
                   | nm `elem` ["Clash.Clash.Annotations.BitRepresentation.Deriving.dontApplyInHDL"
                               ]
                   = [0,1]
                   | otherwise
...
appPropFast ctx@(TransformContext is _) = \case
  e@App {}   -> uncurry (go is) (collectArgs e)
  e@TyApp {} -> uncurry (go is) (collectArgs e)
  e          -> return e
 where
  go is0 (collectArgs -> (fun,args0@(_:_))) args1 =
    go is0 fun (args0 ++ args1)
  ...
  go is0 (TyLam tv e) (Right t:args) = do
    setChanged
    is1 <- unionInScope is0 <$> Lens.use globalInScope
    let subst = extendTvSubst (mkSubst is1) tv t
  ...
  case tyView eTy of
    ...
    tv -> case nm of
      ...
      "Clash.Sized.Vector.dfold" | length args == 8 ->
        ...
        in  case runExcept (tyNatSize tcm nTy) of
          Right n -> reduceDFold c n aTy fun start arg
          _ -> return e
      ...
      "Clash.Sized.Vector.dtfold" | length args == 8 ->
        ...
        in  case runExcept (tyNatSize tcm nTy) of
          Right n -> reduceDTFold c n aTy lrFun brFun arg
          _ -> return e
      ...
      "Clash.Sized.RTree.tdfold" | length args == 8 ->
        let ([_kn,_motive,lrFun,brFun,arg],[_mTy,nTy,aTy]) = Either.partitionEithers args
        in  case runExcept (tyNatSize tcm nTy) of
          Right n -> reduceTFold c n aTy lrFun brFun arg
          _ -> return e
