...
constantSpec ctx@(TransformContext is0 _) e@(App e1 e2)
  ...
  = do e2Speccable <- canConstantSpec is0 e2
       e2isConstant <- isConstantNotClockReset e2
       if traceIf (e2Speccable && not e2isConstant) "WOULD NOT HAVE SPECCED BEFORE" $ trace "---------------------------------" $ trace "e2" $ trace (showPpr e2) $ trace "is speccable" $ traceShowId e2Speccable then
         specializeNorm ctx e
       else
         return e
